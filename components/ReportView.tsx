...
const ReportView: React.FC<ReportViewProps> = ({ candidate, geminiApiKey, showToast }) => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const result = await generatePsychologicalReport(candidate.results, candidate.name, candidate.appliedPosition, geminiApiKey);
      setAiReport(result.sections);
      setRecommendation(result.recommendation);

      // Save report to backend
      try {
        await fetch('/api/save_report.php', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            // Assuming we have the admin token available or can get it from storage. 
            // Since ReportView is used in Admin context, we can import authService.
            'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
          },
          body: JSON.stringify({
            participantId: candidate.id,
            aiReport: result.sections,
            recommendation: result.recommendation
          })
        });
        showToast("Laporan disimpan ke database.", "success");
      } catch (saveError) {
        console.error("Failed to save report", saveError);
        showToast("Laporan dibuat tapi gagal disimpan.", "info");
      }
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start border-b-4 border-slate-900 pb-10 gap-8 relative">
        <div className="flex gap-8 items-center">
          <div className="w-24 h-24 rounded-[2rem] flex items-center justify-center relative overflow-hidden bg-white">
            <img src="./images/logo.png" alt="Buana Megah Logo" className="w-full h-full object-contain" />
          </div>
          <div>
